import Image from "next/image";
import { LiaDumbbellSolid, LiaHeartbeatSolid, LiaStopwatchSolid, LiaUserNinjaSolid } from "react-icons/lia";

const Features = () => {
    return (
        <div className="bg-[url(/featuresCover.jpg)] h-screen flex items-center justify-between mt-16 px-5 lg:px-20">
            <div className="w-1/2">
                <div className="border-l-2 border-[#EE6C0C] pl-6">
                    <h4 className="font-medium text-xl">Awsome Features</h4>
                    <h2 className="font-bold uppercase text-[#EE6C0C] text-3xl">Why Choose Us</h2>
                </div>
                <div className="flex pt-6 justify-between items-center">
                    <div className="flex gap-4">
                        <LiaDumbbellSolid className="text-6xl" />
                        <div>
                            <h3 className="text-xl font-black">Pro Equipment</h3>
                            <p className="w-[223px] pt-2">Duis sed scelerisque nibh. Fusce efficitur ante sit tincidunt.</p>
                        </div>
                    </div>
                    <div className="flex gap-4">
                        <LiaUserNinjaSolid className="text-6xl" />
                        <div>
                            <h3 className="text-xl font-black">Ninja Coach</h3>
                            <p className="w-[223px] pt-2">Duis sed scelerisque nibh. Fusce efficitur ante sit tincidunt.</p>
                        </div>
                    </div>
                </div>
                <div className="flex pt-6 justify-between items-center">
                    <div className="flex gap-4">
                        <LiaHeartbeatSolid className="text-6xl" />
                        <div>
                            <h3 className="text-xl font-black">Cardio First</h3>
                            <p className="w-[223px] pt-2">Duis sed scelerisque nibh. Fusce efficitur ante sit tincidunt.</p>
                        </div>
                    </div>
                    <div className="flex gap-4">
                        <LiaStopwatchSolid className="text-6xl" />
                        <div>
                            <h3 className="text-xl font-black">Flexible Time</h3>
                            <p className="w-[223px] pt-2">Duis sed scelerisque nibh. Fusce efficitur ante sit tincidunt.</p>
                        </div>
                    </div>
                </div>
            </div>
            <div className="flex items-center relative text-center justify-center flex-col">
                <h1 className=" uppercase lg:text-7xl text-5xl leading-[50px] lg:leading-[70px] font-extrabold"><span className="text-[#EE6C0C]">work out</span> <br /> right now</h1>
                <div className="absolute space-y-6">
                    <Image className=" w-full" src={'/features.png'} width={350} height={300} alt="banner" />
                </div>
            </div>
        </div>
    );
};

export default Features;